...
from insert import insert_score

from userlevel import get_user_level
...
get_level = get_user_level(name)

if get_level is None:
    get_level = 1
else:
    print("current level is: ", get_level)
paused = False
while not done:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            
            insert_score(name, score, level)
            done = True
    ...
    if paused:
        ...
        screen.blit(pause_text, (maxw // 2 - 200, maxh // 2))
        pygame.display.flip()
    ...
    name_text = font.render(f"Player: {name}", True, ORANGE)
